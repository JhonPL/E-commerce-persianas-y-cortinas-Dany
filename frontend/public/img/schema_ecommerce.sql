...
-- Guía instructiva de medidas por categoría (RF-12, RF-16)
CREATE TABLE GuiaMedidas (
    guia_id          SERIAL PRIMARY KEY,
    categoria_id     INT UNIQUE REFERENCES Categorias(categoria_id),
    instrucciones    TEXT,                           -- Texto explicativo de cómo medir el vano
    video_url        VARCHAR(255),                   -- URL YouTube / Vimeo del video instructivo
    margen_ancho_cm  INT DEFAULT 0,                 -- Cm adicionales de ancho recomendados (por definir)
    margen_alto_cm   INT DEFAULT 0                  -- Cm adicionales de alto recomendados (por definir)
);
